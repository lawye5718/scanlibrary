...
| 选项 | 建议值 | 说明 |
|---|---|---|
| OCR 后端 | `glm-ocr` | 默认；`mineru` 需你本机装过 MinerU CLI；`text-layer` 只适用于本来就可选中文字的 PDF |
| OCR 模型 | `glm-ocr` | 也可换成 Ollama 里其他视觉模型（如 `qwen2.5vl`） |
| Ollama 地址 | `http://localhost:11434` | 默认 |
| 渲染 DPI | 200（清晰扫描件）／300（字小、发虚） | 越高越准越慢 |
| 并发页数 | 2（16GB 内存填 1–2，32GB 可填 3） | Ollama 会排队，填太大只是占内存 |
| 页面范围 | 试跑填 `1-10` | 留空=全书 |
...
以下参数在本项目真实跑过的书（400 页传记、图注密排书、试读样本）上调优得出，
已经内置为 `server.py` 里的 `RECOMMENDED_DEFAULTS`——页面留空的项自动按这套补齐，
人工填了的值永远优先。**开箱即用，不必逐项研究。**

| 参数 | 推荐值 | 何时调整 |
|---|---|---|
| 渲染 DPI | **200** | 原书字号极小（缩印古籍、口袋本）→ 300；扫描非常清晰 → 可试 150 提速 |
| OCR 后端 | **glm-ocr** | 复杂版面（表格/公式多）→ paddle-layout；PDF 本身可选中文本 → text-layer |
| OCR 并发 | **1** | glm-ocr 已吃满 GPU，加并发只会互相拖慢；前端换 paddle-layout 后可试 2 |
| 校对开关 | **先关，出书后单独校对** | 校对很慢（300 页 +1~2 小时），先看 OCR 质量再决定 |
| 校对模型 | **qwen14b-pro** | 换成你已装的任意 7B+ 文本模型 |
| 校对分块字数 | **700** | 模型长文不稳（丢字/复读）→ 降到 500 |
| 跨页去重相似度 | **0.90** | 漏了重复段 → 0.85；误伤正常段 → 0.95 |
| 段首前缀去重阈值 | **>4 字**（≥5 才动刀） | 中文段落自然共有前缀很少超 4 字；误删 → 调大 |
| 自动结构页识别 | **开** | 封面/封底/扉页/版权/目录页自动划出；识别错了在人工分章面板覆盖 |
| 正文分章方式 | **按目录页** | 目录识别不稳 → 按页数截取（默认每 20 页一章，自动顺延到段落结束） |
| 每章目标页数 | **20** | 章节很长的书 → 30~40 |

**推荐节奏**（也是自动默认值走出来的流程）：

1. `1-10` 试读跑一遍 → 看 EPUB 结构、段落、目录页识别（日志里会打印"自动识别结构页：封面=1、扉页=2…"）；
2. 满意 → 留空页面范围重跑全书（前 10 页复用缓存）；
3. 出书后在任务卡片上单独点"开始校对"，对照表逐条人工确认，不满意一键回退。
